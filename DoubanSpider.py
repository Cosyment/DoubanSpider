...
from db.dao.movie import MovieDao
...
class Spider:
    ...
    def foreachPageUrl(self, page):
        ...
        try:
            ...
            while i < (len(a)):
                ...
                flag = self.getTagDetail(EnumUtil.TagsType.MOVIE, ranking=count, title=title, url=link,
                                         insert_type=EnumUtil.InsertType.BEST)
                sleep(3)  # 休息两秒 应付反爬
                if flag == 1:
                    i += 1
                    count += 1
    ...
    def searchMovie(self, name):
        ...
        try:
            ...
            a.click()  # 点击连接
            # 获取电影详细信息
            self.getTagDetail(tags=EnumUtil.TagsType.MOVIE, url=url, insert_type=EnumUtil.InsertType.HOT)
    ...
    def getLatestReleases(self):
        ...
        try:
            ...
            for i in range(len(l)):
                ...
                # 获取电影详细信息
                self.getTagDetail(tags=EnumUtil.TagsType.MOVIE, url=l[i], insert_type=EnumUtil.InsertType.LATEST)
                print()
        except Exception as e:
            print(e)
        # drive.close()#关闭页面
    ...
    def getHots(self):
        ...
        try:
            ...
            for i in range(len(l)):
                ...
                print("第{}个".format(i + 1))
                # 获取电影详细信息
                self.getTagDetail(tags=EnumUtil.TagsType.MOVIE, url=l[i], insert_type=EnumUtil.InsertType.HOT)
                print()
            drive.implicitly_wait(3)  # 等待3秒
        except Exception as e:
            print(e)
    ...
    def getTagByApi(self, tags, sort=EnumUtil.SortType.ALL.value):
        ...
        try:
            ...
            if tags == EnumUtil.TagsType.MOVIE:
                record_type = EnumUtil.CrawlRecordType.MOVIE.value
            elif tags == EnumUtil.TagsType.TELEVISION:
                record_type = EnumUtil.CrawlRecordType.TELEVISION.value
            elif tags == EnumUtil.TagsType.ANIMATION:
                record_type = EnumUtil.CrawlRecordType.ANIMATION.value
            else:
                record_type = EnumUtil.CrawlRecordType.VARIETY.value
    ...
    def getTagDetail(self, tags,  # TagType
        ...
        try:
            ...
            if self.insert_enabled:
                if tags == EnumUtil.TagsType.MOVIE:
                    MovieDao.insert(insert_type=insert_type, ranking=ranking, title=title, alias=name,
                                    language=language, cover=cover,
                                    rating=rating, year=year, director=director, writer=writer, actors=actors,
                                    type=type, release_date=release_date,
                                    area=area, duration=duration, introduction=text, trailer_url=trailer)
                elif tags == EnumUtil.TagsType.TELEVISION:
    ...
    try:
        ...
        if s != 0:
            if i == 1:
                ...
                s.getTagByApi(EnumUtil.TagsType.MOVIE)
            elif i == 3:
                s.getTagByApi(EnumUtil.TagsType.MOVIE, EnumUtil.SortType.LATEST.value)
            elif i == 4:
                s.getTagByApi(EnumUtil.TagsType.MOVIE, EnumUtil.SortType.HOT.value)
            elif i == 5:
...
def tags_menu(tags):
    ...
    try:
        ...
        if s != 0:
            if i == 1:
                s.getTagByApi(tags, EnumUtil.SortType.ALL.value)  # 获取全部（默认热门）
            elif i == 2:
                s.getTagByApi(tags, EnumUtil.SortType.LATEST.value)  # 获取豆瓣最新上映
            elif i == 3:
                s.getTagByApi(tags, EnumUtil.SortType.HOT.value)  # 获取热门
            elif i == 4:
                s.searchMovie('热门')
        else:
            print("操作有误，重新操作")
